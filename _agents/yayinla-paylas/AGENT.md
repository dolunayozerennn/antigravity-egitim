...
- Deploy edilen projede `Sistem_Nasil_Calisir.html` olarak kaydet
...
### Adım B6: Görselleştirme (Opsiyonel)
- Kullanıcı isterse `_skills/proje-gorsellestirici/SKILL.md` ile projenin akış şemasını HTML olarak oluştur
- Paylaşım paketine dahil et

---

## 🎨 Bölüm C — Görselleştirme (Bağımsız Akış)

Bu akış `_skills/proje-gorsellestirici/SKILL.md` yönergesini takip eder.

1. Hedef projeyi analiz et
2. 3-6 temel adıma böl (teknik olmayan, müşteri-dostu dil)
...
6. Kullanıcıya bildir — Space tuşuyla adım adım gösterebilir
...
Bu agent'ın iki alt workflow'u vardır:
...
```
Kullanıcı: "Sweatcoin botunu deploy et ve nasıl çalıştığını gösteren bir sayfa yap"
Agent: Önce Deploy (A1-A5), sonra Görselleştirme (C1-C6)
```

### Senaryo 4: Skill Paylaşımı
```
Kullanıcı: "website-olusturucu skill'ini paylaş"
Agent: Export akışı Mod 1 → skill temizle + paketle
```
