...
2. **Her marka için kişiselleştir:**
   ...
   - `{portfolio_url}` → KULLANICI'nın portfolyo linki
   - Konu satırı: `"{marka_adi} x KULLANICI_ADI_BURAYA | İş Birliği Teklifi"`
...
3. **Gönderim komutu:**
   ...
   python3 _skills/eposta-gonderim/scripts/send_email.py \
     --to "{email}" \
     --subject "{marka_adi} x KULLANICI_ADI_BURAYA | İş Birliği Teklifi" \
