...
## 🛠 Kullanım Amacı
Kullanıcının tek yapması gereken markayı ve fiyatı vermektir — ya da "e-postadan bak" demektir. Fatura üzerindeki *Description* kısmında doğrudan "`[Brand]` collaboration with @INSTAGRAM_HANDLE_BURAYA" formatı uygulanır. İsimlendirme ise özel olarak `INVOICE_[Marka]_[GG-AA-YYYY].pdf` şeklinde yapılır.
...
// turbo
```bash
open -R "./_skills/fatura-olusturucu/uretilen-faturalar/INVOICE_${SAFE_BRAND}_${DATE}.pdf"
```

3. Ayrıca kullanıcıya dosyanın tıklanabilir linkini ver: `[INVOICE_Marka_GG-AA-YYYY.pdf](file://./_skills/fatura-olusturucu/uretilen-faturalar/INVOICE_Marka_GG-AA-YYYY.pdf)`
