"""
Bildirim Modülü — v4 (Gmail API + Telegram)
...
def build_message_text(lead_data: dict) -> str:
    """Lead sözlüğünü sadece istenen 4 bilgiyi içerecek şekilde formatlar."""
    ...
    phone = data_lower_keys.get("phone", "-")
...
def send_email_notification(lead_data: dict, plain_text: str) -> bool:
    ...
    for attempt in range(_MAX_NOTIFY_RETRIES):
        try:
            ...
            logger.info(
                f"✅ E-Posta gönderildi → {Config.NOTIFY_EMAIL} | "
