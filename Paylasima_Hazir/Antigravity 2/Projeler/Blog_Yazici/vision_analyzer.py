...
if not GROQ_API_KEY:
    print("HATA: GROQ_API_KEY master.env dosyasında bulunamadı!")
    sys.exit(1)
...
SCRIPT_TEXT = """
Başlık: Typeless 5 — Mac/Windows Speak to Edit
...
def analyze_batch(frames_batch, batch_idx, total_batches):
    ...
    prompt = f"""Bu {len(frames_batch)} frame bir ekran kaydından alındı. Bu video "Typeless" adlı bir yapay zeka aracının ekran kaydını gösteriyor. Typeless, metin düzenleme işlemlerini sesli komutlarla yapılmasını sağlayan bir araç.
...
def main():
    frames_dir = sys.argv[1] if len(sys.argv) > 1 else "./Projeler/Blog_Yazici/typeless5/frames"
    ...
    for batch_idx, batch in enumerate(batches):
        result, usage = analyze_batch(batch, batch_idx, len(batches))
