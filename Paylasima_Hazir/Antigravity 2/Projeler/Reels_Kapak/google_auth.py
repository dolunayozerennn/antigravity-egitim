...
Kullanım:
    ...
    # Markanın_Adı hesabı (EMAIL_ADRESI_BURAYA)
    gmail = get_gmail_service("primary")
    sheets = get_sheets_service("primary")

    # KULLANICI_ADI_BURAYA AI hesabı (KULLANICI_EMAIL_BURAYA)
    gmail = get_gmail_service("secondary")
    sheets = get_sheets_service("secondary")
    drive = get_drive_service("secondary")

Token dosyaları:
    _knowledge/credentials/oauth/gmail-outreach-token.json
    _knowledge/credentials/oauth/gmail-primary-token.json
    _knowledge/credentials/oauth/gmail-secondary-token.json
"""
...
def _get_credentials(account: str = "outreach") -> Credentials:
    ...
    Args:
        account: "outreach" (EMAIL_ADRESI_BURAYA), "primary" (EMAIL_ADRESI_BURAYA)
                 veya "secondary" (KULLANICI_EMAIL_BURAYA)
