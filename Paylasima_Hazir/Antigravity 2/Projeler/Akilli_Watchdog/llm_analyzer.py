...
Bu verilerin kalitesini değerlendir."""
    ...
    def analyze_pipeline_consistency(
        ...
        notion_entry_count: int,
        pipeline_type: str,
    ) -> dict:
        """
        Sheet ve Notion arasındaki veri tutarlılığını LLM ile analiz eder.

        Returns:
            {"status": "OK"|"WARNING"|"CRITICAL", "analysis": str}
        """
        # Basit kural tabanlı kontrol — LLM gereksiz
        # Ama yorum ve bağlam için LLM kullanılabilir
        gap = abs(sheets_row_count - notion_entry_count)
        gap_percent = (gap / max(sheets_row_count, 1)) * 100
        ...
        if gap_percent > 30:
            status = "CRITICAL"
            analysis = (
                f"🚨 Sheets ({sheets_row_count} satır) ve Notion ({notion_entry_count} kayıt) "
                f"arasında %{gap_percent:.0f} fark var. "
                f"Pipeline'da ciddi bir sorun olabilir — lead'ler kaçırılıyor olabilir!"
            )
        elif gap_percent > 10:
            status = "WARNING"
            analysis = (
                f"⚠️ Sheets ({sheets_row_count}) ve Notion ({notion_entry_count}) "
                f"arasında %{gap_percent:.0f} fark var. "
                f"Duplikasyon filtreleri veya geçici hatalar bu farka sebep olabilir."
            )
        else:
            status = "OK"
            analysis = (
                f"✅ Sheets ({sheets_row_count}) ve Notion ({notion_entry_count}) "
                f"tutarlı (fark: %{gap_percent:.0f})."
            )
    ...
    def full_analysis(
        ...
        if notion_result and pipeline == "sheets_to_notion":
            # Toplam satır sayısını tüm tab'lardan topla
            total_sheets = sum(
                td.get("total_rows", 0)
