...
def run(tab_name="In EN, Roblox", dry_run=False, limit=100, validate_only=False):
    ...
    print("\n🔐 Auth: Kaynak (outreach) + Hedef (primary) oturum açılıyor...")
    sheets_outreach = get_sheets_service("outreach")
    sheets_primary = get_sheets_service("primary")
    ...
    existing_urls = get_existing_target_urls(sheets_primary, tab_name)
    ...
    try:
        appended = append_rows_to_target(sheets_primary, tab_name, batch, today)
