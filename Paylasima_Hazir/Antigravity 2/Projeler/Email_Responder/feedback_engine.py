...
================================================================
Bu script:
1. Gmail'den gerçek thread çiftlerini çeker (creator → KULLANICI_ADI_BURAYA yanıtı)
2. AI agent'ı aynı mesaja nasıl draft yazacağını simüle eder
3. KULLANICI_ADI_BURAYA'ın gerçek yanıtıyla AI draft'ını Groq LLM ile karşılaştırır
...
def extract_thread_pairs(service, days=30, limit=20):
    """
    Gmail'den 'creator yazdı → KULLANICI_ADI_BURAYA cevap verdi' çiftlerini çek.
    ...
    Her çift:
    {
        ...
        "kullanici_reply": "...",       # KULLANICI_ADI_BURAYA'ın gerçek yanıtı
        "subject": "...",
    ...
    # CALISAN_EMAIL_BURAYA'in yanıtladığı thread'leri bul
    query = f"from:CALISAN_EMAIL_BURAYA newer_than:{days}d"
    ...
    for msg_meta in messages:
        ...
        try:
            ...
            # "Creator yazdı → KULLANICI_ADI_BURAYA cevap verdi" çiftlerini bul
            for i in range(len(thread_messages) - 1):
                ...
                # Creator yazdı → KULLANICI_ADI_BURAYA cevap verdi
                if 'sirket.com' not in current_sender and 'sirket.com' in next_sender:
                    creator_body = extract_body(current_msg['payload'])
                    kullanici_body = extract_body(next_msg['payload'])
                    
                    # Boş body'leri atla
                    if not creator_body.strip() or not kullanici_body.strip():
                        continue
                    ...
                    elif 'sirket.com' in first_sender:
                    ...
                    pairs.append({
                        ...
                        "kullanici_reply": kullanici_body[:3000],
                        "subject": subject,
...
# ══════════════════════════════════════════════════════════════════════
# ADIM 3: AI Draft vs KULLANICI_ADI_BURAYA Gerçek Yanıt Karşılaştırması
# ══════════════════════════════════════════════════════════════════════

def compare_with_human(pair, ai_result):
    """
    AI'ın ürettiği draft ile KULLANICI_ADI_BURAYA'ın gerçek yanıtını karşılaştır.
...
email draft with the ACTUAL email written by a human (KULLANICI_ADI_BURAYA from MARKA_ADI_BURAYA).
...
Respond in JSON:
{
    ...
    "scores": {
        ...
        "tone_match": 1-10,         // Is the tone similar to KULLANICI_ADI_BURAYA's style?
...
KULLANICININ ACTUAL REPLY:
{pair['kullanici_reply'][:1500]}
...
def run_backtesting(days=30, limit=15):
    ...
    for i, pair in enumerate(pairs):
        ...
        # Karşılaştırma
        print(f"   📊 KULLANICI_ADI_BURAYA vs AI karşılaştırılıyor...")
        ...
        result_entry = {
            ...
            "human_reply_preview": pair["kullanici_reply"][:300],
