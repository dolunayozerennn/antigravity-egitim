...
Bu agent:
- Creator MARKA_ADI_BURAYA uygulamasını tanıtıyor
...
# ══════════════════════════════════════════════════════
IP_KNOWLEDGE = """
BUSINESS: MARKA_ADI_BURAYA Influencer Program — Performance-bazlı affiliate model.
- Creator MARKA_ADI_BURAYA uygulamasını kendi kitlelerine tanıtıyor
...
KULLANICININ GERÇEK YAKLAŞIMI (backtesting'den öğrenildi):
...
ONBOARDING ADIMLARI:
1. MARKA_ADI_BURAYA uygulamasını indir (ücretsiz)
2. Hesap oluştur
...
1. Download the free MARKA_ADI_BURAYA app and create an account.
...
Thank you for reaching out about this. To look into this for you, could you please share the email address you used to sign up for MARKA_ADI_BURAYA?
    ...
    def handle(self, ctx):
        ...
        if intent_result and intent_result.get("confidence", 0) >= 0.6:
            ...
            try:
                ...
                send_alert(
                    "Email Email Responder: LLM Intent Tespiti Başarısız (IP Agent)",
