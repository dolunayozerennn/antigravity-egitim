...
Desteklenen ortamlar:
  ...
  - Railway/Cloud: GOOGLE_OUTREACH_TOKEN_JSON veya GOOGLE_PRIMARY_TOKEN_JSON
...
Kullanım:
    ...
    # MARKA_ADI_BURAYA hesabı (CALISAN_EMAIL_BURAYA)
    gmail = get_gmail_service("primary")
    sheets = get_sheets_service("primary")

Token dosyaları:
    _knowledge/credentials/oauth/gmail-outreach-token.json
    _knowledge/credentials/oauth/gmail-primary-token.json
"""
...
TOKEN_FILES = {
    ...
    "primary": "gmail-primary-token.json",
    "secondary": "gmail-secondary-token.json",
}

# Hesap → Railway environment variable eşlemesi
TOKEN_ENV_VARS = {
    "outreach": "GOOGLE_OUTREACH_TOKEN_JSON",
    "primary": "GOOGLE_PRIMARY_TOKEN_JSON",
    "secondary": "GOOGLE_SECONDARY_TOKEN_JSON",
}
...
def _get_credentials(account: str = "outreach") -> Credentials:
    ...
    Args:
        account: "outreach" (EMAIL_ADRESI_BURAYA) veya "primary" (CALISAN_EMAIL_BURAYA)
...
def gmail_primary():
    """CALISAN_EMAIL_BURAYA Gmail service — kısa yol."""
    return get_gmail_service("primary")
...
def sheets_primary():
    """CALISAN_EMAIL_BURAYA Sheets service — kısa yol."""
    return get_sheets_service("primary")


def gmail_secondary():
    """KULLANICI_EMAIL_BURAYA Gmail service — kısa yol."""
    return get_gmail_service("secondary")


def sheets_secondary():
    """KULLANICI_EMAIL_BURAYA Sheets service — kısa yol."""
    return get_sheets_service("secondary")


def drive_secondary():
    """KULLANICI_EMAIL_BURAYA Drive service — kısa yol."""
    return get_drive_service("secondary")
