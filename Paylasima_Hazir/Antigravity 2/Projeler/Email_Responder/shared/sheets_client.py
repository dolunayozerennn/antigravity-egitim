...
Spreadsheet: https://docs.google.com/spreadsheets/d/SPREADSHEET_ID_BURAYA
...
SPREADSHEET_ID = 'SPREADSHEET_ID_BURAYA'
SOURCE_SPREADSHEET_ID = 'SOURCE_SPREADSHEET_ID_BURAYA'

# Header cache — her seferinde API çağrısı yapmamak için
_headers_cache = {}


def authenticate_sheets(account: str = "primary"):
    """
...
def get_email_template(sheets_service, niche_index=0):
    ...
    if not template["subject"]:
        template["subject"] = "Collaboration Inquiry - MARKA_ADI_BURAYA"
...
def get_existing_target_urls(sheets_service_primary, tab_name="In EN, Roblox"):
    """
    ...
    """
    req = sheets_service_primary.spreadsheets().values().get(
        spreadsheetId=SPREADSHEET_ID,
...
def append_rows_to_target(sheets_service_primary, tab_name, rows_data, fetch_date):
    """
    ...
    req = sheets_service_primary.spreadsheets().values().append(
        spreadsheetId=SPREADSHEET_ID,
...
def update_fetched_date(sheets_service_primary, tab_name, row, date_str):
    """
    Belirli bir satırın Fetched (G) sütununu güncelle.
    """
    col_map = _get_column_map(sheets_service_primary, tab_name)
    ...
    req = sheets_service_primary.spreadsheets().values().update(
        spreadsheetId=SPREADSHEET_ID,
        range=range_cell,
        valueInputOption="USER_ENTERED",
...
def update_reached_out_date(sheets_service_primary, tab_name, row, date_str):
    """
...
def get_fetched_yesterday_contacts(sheets_service_primary, tab_name, yesterday_str):
    """
