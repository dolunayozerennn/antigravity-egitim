...
def check_for_alerts():
    ...
    pending = get_pending_notifications(videos, days_threshold=7)
    ...
    for item in pending:
        ...
        # 14 günden fazla oldu ve kırmızı bildirim (seviye 2) atılmadıysa
        if days_passed >= 14 and notified_level < 2:
            subject = f"🔴 KRİTİK: Ödeme Uyarısı - {item['title']}"
            color = "#ff4d4f"
            bg_color = "#fff1f0"
            headline = "🚨 14 Günü Geçen Kritik Tahsilat Bildirimi"
            new_level = 2
        # 7 ile 14 gün arası ve sarı bildirim (seviye 1) atılmadıysa
        elif days_passed >= 7 and days_passed < 14 and notified_level < 1:
            subject = f"🟡 Geciken Ödeme: {item['title']}"
            color = "#faad14"
            bg_color = "#fffbe6"
            headline = "⚠️ 7 Günü Geçen Tahsilat Bildirimi"
            new_level = 1
        else:
