...
Bildirim Seviyeleri:
  ...
  1 = Sarı uyarı (7 gün) gönderildi
  2 = Kırmızı uyarı (14 gün) gönderildi
"""

from datetime import datetime
from notion_client import get_notification_level_from_comments
...
def mark_as_notified(record_id, level):
    """
    Bildirim seviyesini in-memory cache'e kaydeder ve loglar.
    Notion'a yorum ATMAZ (kullanıcı talebi: sadece e-mail bildirimi yeterli).
    
    Args:
        record_id: Notion page ID
        level: 1=sarı (7 gün), 2=kırmızı (14 gün)
    """
    today = datetime.now().strftime("%Y-%m-%d")
    
    if level == 1:
        print(f"[LOG] Sarı uyarı (seviye 1) e-posta gönderildi — page={record_id} ({today})")
    elif level == 2:
        print(f"[LOG] Kırmızı uyarı (seviye 2) e-posta gönderildi — page={record_id} ({today})")
    else:
        return False
...
def get_pending_notifications(videos, days_threshold=7):
    """
    ...
    Args:
        ...
        days_threshold: Minimum gün eşiği (varsayılan: 7)
