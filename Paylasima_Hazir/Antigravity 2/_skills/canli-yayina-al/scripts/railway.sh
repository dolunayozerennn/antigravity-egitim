...
ANTIGRAVITY_ROOT="$(cd "$(dirname "$0")/../../.." && pwd)"
...
case "$1" in
    ...
    create-service)
        if [ -z "$4" ]; then
            ...
            echo "   Örnek: ./railway.sh create-service abc-123 my-bot GITHUB_USERNAME_BURAYA/my-bot main" >&2
            exit 1
        fi
        local_proj="$2"
        local_svc="$3"
    ...
    connect-repo)
        if [ -z "$3" ]; then
            ...
            echo "   Örnek: ./railway.sh connect-repo abc-123 GITHUB_USERNAME_BURAYA/my-bot main" >&2
            exit 1
        fi
        local_svc_id="$2"
        local_repo="$3"
