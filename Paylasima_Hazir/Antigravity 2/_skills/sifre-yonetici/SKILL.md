...
├── oauth/
│   ├── gmail-hesap1-token.json
│   └── gmail-hesap2-token.json
└── README.md
...
| Gmail (İkincil) | `GMAIL_SECONDARY_CLIENT_ID`, `GMAIL_SECONDARY_CLIENT_SECRET` | |
...
├── gmail-outreach-token.json   ← Birincil hesap (Gmail+Drive+Sheets)
├── gmail-secondary-token.json  ← İkincil hesap (Gmail+Drive+Sheets)
├── gmail-outreach-credentials.json
└── gmail-secondary-credentials.json
```
...
cd _knowledge/credentials/oauth && python3 auth_helper.py secondary
```
