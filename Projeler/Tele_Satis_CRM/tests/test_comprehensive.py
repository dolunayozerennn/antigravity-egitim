...
def test_e2e_real_sheet_row():
    ...
    raw = {
        ...
        "email": "bariscemdogan123@gmail.com",
    ...
    # Email küçük harf mi?
    assert cleaned["clean_email"] == "bariscemdogan123@gmail.com"
...
def test_e2e_local_format_phone():
    ...
    raw = {
        "full_name": "Hüseyin Samat",
        "email": "huseyinsamat@hotmail.com",
...
def test_config_tab_names():
    ...
    assert "Mart-2026-Saat Bazlı-v2" in tab_names
    assert len(tab_names) == 1
    print("  ✅ config — tab tanımları")

def test_config_spreadsheet_id():
    """Spreadsheet ID doğru mu?"""
    from config import Config
    assert Config.SPREADSHEET_ID == "10uTCr65VlIBng0Sxlmz7h1Q2y5AWBp1qDQHTeqI7mGU"
    print("  ✅ config — spreadsheet ID")

def test_config_notion_db():
    """Notion database ID doğru mu?"""
    from config import Config
    assert Config.NOTION_DATABASE_ID == "31226924bb82800e9adad6f16399eba0"
    print("  ✅ config — Notion database ID")
