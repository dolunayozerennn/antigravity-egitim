...
- **Konuşma hafızası:** Her kullanıcı (subscriber_id bazlı) için son 10 mesajı bir veritabanında (örn. `loglar/chat.db` SQLite) sakla. Sunucu yeniden başladığında konuşmaların silinmemesi için geçici bellek (RAM) **kullanma**. Bu sayede AI, önceki mesajların bağlamını hep bilir.
- **Yapılandırılmış çıktı:** AI'dan şu JSON formatında yanıt iste. Kesinlikle bir JSON parse/temizleme işleminden geçir (veya modelin Structured Outputs/JSON Mode özelliğini kullan) ki başındaki ve sonundaki Markdown kod blokları (` ```json ` vb.) sistemi bozmasın:
```json
{
...
Bu klasörü AntiGravity'ye (veya kullandığınız AI asistanına) bağlayın ve şu komutu verin:

> "Bu klasördeki INSTRUCTIONS.md dosyasını oku ve sistemi adım adım inşa et. Ayrıca projeyi Railway'de çalıştırabilmem için gerekli olan `requirements.txt`, `.gitignore` ve `Procfile` gibi altyapı dosyalarını da oluşturmayı unutma."
